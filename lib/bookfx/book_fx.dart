...
class _BookFxState extends State<BookFx> with SingleTickerProviderStateMixin {
  ...
  _controller = AnimationController(
    ...
    NetworkImage("${Strings.urlMain}/${widget.images.elementAt(widget.controller.currentIndex + 1)['images']}").resolve(new ImageConfiguration()).addListener(
...
class _BookPainter extends CustomPainter {
  ...
  _BookPainter(this.p, this.images) : super(repaint: p);
    ...
    if (p.value.a != p.value.f) {
      if (p.value.a.y == p.value.f.y) {
        ...
        if (!p.value.b.x.isNaN) {
          ...
          canvas.drawPath(
              mPathB, paint..shader = ui.ImageShader(images, TileMode.decal, TileMode.decal, Matrix4.identity().scaled(0.5, 0.5).storage));
